---
title: 嘉兴-立体库-采购入库
desc: 嘉兴-立体库-采购入库
type: flow
navIgnore: false
tags:
  - wms/flow/jx_ltk_cgrk
---

# title
>[!workflow] jx_ltk_cgrk
> 嘉兴-立体库-采购入库
> 
> 部署号: N/A
>
> 最后部署时间: 2024-03-05 14:22:03.856646
> 
> 版本: 
> 
> 定义号: 


# 图形内容
```xml
<?xml version="1.0" encoding="UTF-8"?>
<bpmn:definitions xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:di="http://www.omg.org/spec/DD/20100524/DI" id="Definitions_1" targetNamespace="http://bpmn.io/schema/bpmn">
  <bpmn:process id="jx_ltk_cgrk" name="嘉兴-立体库-采购入库" isExecutable="true">
    <bpmn:startEvent id="StartEvent_1">
      <bpmn:outgoing>Flow_1uxx9ml</bpmn:outgoing>
    </bpmn:startEvent>
    <bpmn:sequenceFlow id="Flow_1uxx9ml" sourceRef="StartEvent_1" targetRef="Activity_115idnq" />
    <bpmn:exclusiveGateway id="Gateway_1kik59l">
      <bpmn:incoming>Flow_0dw1gfd</bpmn:incoming>
      <bpmn:outgoing>Flow_0vtrbcl</bpmn:outgoing>
      <bpmn:outgoing>Flow_0g0qdv8</bpmn:outgoing>
    </bpmn:exclusiveGateway>
    <bpmn:sequenceFlow id="Flow_11upx3q" sourceRef="Gateway_1r7304v" targetRef="Activity_07r2dk2" />
    <bpmn:serviceTask id="Activity_115idnq" name="下发RCS指令">
      <bpmn:incoming>Flow_1uxx9ml</bpmn:incoming>
      <bpmn:incoming>Flow_178xmy2</bpmn:incoming>
      <bpmn:outgoing>Flow_181n368</bpmn:outgoing>
    </bpmn:serviceTask>
    <bpmn:userTask id="Activity_07r2dk2" name="BCR扫托盘码">
      <bpmn:incoming>Flow_11upx3q</bpmn:incoming>
      <bpmn:outgoing>Flow_0dw1gfd</bpmn:outgoing>
    </bpmn:userTask>
    <bpmn:sequenceFlow id="Flow_0dw1gfd" sourceRef="Activity_07r2dk2" targetRef="Gateway_1kik59l" />
    <bpmn:sequenceFlow id="Flow_0vtrbcl" sourceRef="Gateway_1kik59l" targetRef="Activity_0ies202" />
    <bpmn:sequenceFlow id="Flow_0g0qdv8" sourceRef="Gateway_1kik59l" targetRef="Activity_1o6n5ho" />
    <bpmn:serviceTask id="Activity_1o6n5ho" name="分立体库货位">
      <bpmn:incoming>Flow_0g0qdv8</bpmn:incoming>
      <bpmn:outgoing>Flow_1fpc6bn</bpmn:outgoing>
    </bpmn:serviceTask>
    <bpmn:userTask id="Activity_0zx4wn9" name="WCS问巷道号">
      <bpmn:incoming>Flow_16h9ynn</bpmn:incoming>
      <bpmn:outgoing>Flow_1bwkxbo</bpmn:outgoing>
    </bpmn:userTask>
    <bpmn:sequenceFlow id="Flow_1fpc6bn" sourceRef="Activity_1o6n5ho" targetRef="Activity_0xls0wa" />
    <bpmn:userTask id="Activity_0uzhxju" name="WCS问货位号">
      <bpmn:incoming>Flow_1hfciq2</bpmn:incoming>
      <bpmn:outgoing>Flow_0m5auoc</bpmn:outgoing>
    </bpmn:userTask>
    <bpmn:serviceTask id="Activity_0q6mjmj" name="应答巷道号">
      <bpmn:incoming>Flow_1bwkxbo</bpmn:incoming>
      <bpmn:outgoing>Flow_1hfciq2</bpmn:outgoing>
    </bpmn:serviceTask>
    <bpmn:sequenceFlow id="Flow_1bwkxbo" sourceRef="Activity_0zx4wn9" targetRef="Activity_0q6mjmj" />
    <bpmn:sequenceFlow id="Flow_1hfciq2" sourceRef="Activity_0q6mjmj" targetRef="Activity_0uzhxju" />
    <bpmn:serviceTask id="Activity_0jixr45" name="应答货位号">
      <bpmn:incoming>Flow_0m5auoc</bpmn:incoming>
      <bpmn:outgoing>Flow_1g97zx3</bpmn:outgoing>
    </bpmn:serviceTask>
    <bpmn:sequenceFlow id="Flow_0m5auoc" sourceRef="Activity_0uzhxju" targetRef="Activity_0jixr45" />
    <bpmn:serviceTask id="Activity_0ies202" name="不合格，记录异常">
      <bpmn:incoming>Flow_0vtrbcl</bpmn:incoming>
    </bpmn:serviceTask>
    <bpmn:userTask id="Activity_1fuse87" name="立体库上架完成">
      <bpmn:incoming>Flow_1g97zx3</bpmn:incoming>
      <bpmn:outgoing>Flow_18zcwiw</bpmn:outgoing>
    </bpmn:userTask>
    <bpmn:sequenceFlow id="Flow_1g97zx3" sourceRef="Activity_0jixr45" targetRef="Activity_1fuse87" />
    <bpmn:endEvent id="Event_1a90rs1">
      <bpmn:incoming>Flow_1nq1i8n</bpmn:incoming>
    </bpmn:endEvent>
    <bpmn:sequenceFlow id="Flow_1nq1i8n" sourceRef="Activity_1vc4um7" targetRef="Event_1a90rs1" />
    <bpmn:sequenceFlow id="Flow_18zcwiw" sourceRef="Activity_1fuse87" targetRef="Activity_1vc4um7" />
    <bpmn:serviceTask id="Activity_1vc4um7" name="记录库存,释放预占">
      <bpmn:incoming>Flow_18zcwiw</bpmn:incoming>
      <bpmn:outgoing>Flow_1nq1i8n</bpmn:outgoing>
    </bpmn:serviceTask>
    <bpmn:sequenceFlow id="Flow_16h9ynn" sourceRef="Activity_0xls0wa" targetRef="Activity_0zx4wn9" />
    <bpmn:serviceTask id="Activity_0xls0wa" name="记录任务、预占库位">
      <bpmn:incoming>Flow_1fpc6bn</bpmn:incoming>
      <bpmn:outgoing>Flow_16h9ynn</bpmn:outgoing>
    </bpmn:serviceTask>
    <bpmn:exclusiveGateway id="Gateway_1r7304v">
      <bpmn:incoming>Flow_181n368</bpmn:incoming>
      <bpmn:outgoing>Flow_1tg2ufs</bpmn:outgoing>
      <bpmn:outgoing>Flow_11upx3q</bpmn:outgoing>
    </bpmn:exclusiveGateway>
    <bpmn:sequenceFlow id="Flow_181n368" sourceRef="Activity_115idnq" targetRef="Gateway_1r7304v" />
    <bpmn:sequenceFlow id="Flow_1tg2ufs" sourceRef="Gateway_1r7304v" targetRef="Activity_1swh3tx" />
    <bpmn:serviceTask id="Activity_1swh3tx" name="记录异常指令">
      <bpmn:incoming>Flow_1tg2ufs</bpmn:incoming>
    </bpmn:serviceTask>
    <bpmn:userTask id="Activity_1j4zvbz" name="人工尝试">
      <bpmn:outgoing>Flow_178xmy2</bpmn:outgoing>
    </bpmn:userTask>
    <bpmn:sequenceFlow id="Flow_178xmy2" sourceRef="Activity_1j4zvbz" targetRef="Activity_115idnq" />
  </bpmn:process>
  <bpmndi:BPMNDiagram id="BPMNDiagram_1">
    <bpmndi:BPMNPlane id="BPMNPlane_1" bpmnElement="jx_ltk_cgrk">
      <bpmndi:BPMNShape id="Activity_0ivcnkl_di" bpmnElement="Activity_115idnq">
        <dc:Bounds x="141" y="210" width="100" height="80" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_0vceily_di" bpmnElement="Activity_07r2dk2">
        <dc:Bounds x="141" y="370" width="100" height="80" />
        <bpmndi:BPMNLabel />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_0j6mtlu_di" bpmnElement="Activity_0ies202">
        <dc:Bounds x="540" y="370" width="100" height="80" />
        <bpmndi:BPMNLabel />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Gateway_1kik59l_di" bpmnElement="Gateway_1kik59l" isMarkerVisible="true">
        <dc:Bounds x="345" y="385" width="50" height="50" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_1nrp5oz_di" bpmnElement="Activity_1o6n5ho">
        <dc:Bounds x="141" y="530" width="100" height="80" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_0rwwrlo_di" bpmnElement="Activity_0xls0wa">
        <dc:Bounds x="320" y="530" width="100" height="80" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_02j0mbm_di" bpmnElement="Activity_0zx4wn9">
        <dc:Bounds x="141" y="670" width="100" height="80" />
        <bpmndi:BPMNLabel />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_0h8zye0_di" bpmnElement="Activity_0q6mjmj">
        <dc:Bounds x="320" y="670" width="100" height="80" />
        <bpmndi:BPMNLabel />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_1fpdepd_di" bpmnElement="Activity_0uzhxju">
        <dc:Bounds x="530" y="670" width="100" height="80" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="BPMNShape_0ly44w9" bpmnElement="Activity_0jixr45">
        <dc:Bounds x="750" y="670" width="100" height="80" />
        <bpmndi:BPMNLabel />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_18sih12_di" bpmnElement="Activity_1fuse87">
        <dc:Bounds x="141" y="850" width="100" height="80" />
        <bpmndi:BPMNLabel />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Event_1a90rs1_di" bpmnElement="Event_1a90rs1">
        <dc:Bounds x="352" y="992" width="36" height="36" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_1kh5hx0_di" bpmnElement="Activity_1vc4um7">
        <dc:Bounds x="320" y="850" width="100" height="80" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="BPMNShape_163cdex" bpmnElement="Gateway_1r7304v" isMarkerVisible="true">
        <dc:Bounds x="345" y="225" width="50" height="50" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_0b259e2_di" bpmnElement="Activity_1swh3tx">
        <dc:Bounds x="530" y="210" width="100" height="80" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Activity_04m1i3m_di" bpmnElement="Activity_1j4zvbz">
        <dc:Bounds x="530" y="80" width="100" height="80" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="_BPMNShape_StartEvent_2" bpmnElement="StartEvent_1">
        <dc:Bounds x="173" y="32" width="36" height="36" />
      </bpmndi:BPMNShape>
      <bpmndi:BPMNEdge id="Flow_1uxx9ml_di" bpmnElement="Flow_1uxx9ml">
        <di:waypoint x="191" y="68" />
        <di:waypoint x="191" y="210" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_11upx3q_di" bpmnElement="Flow_11upx3q">
        <di:waypoint x="370" y="275" />
        <di:waypoint x="370" y="320" />
        <di:waypoint x="191" y="320" />
        <di:waypoint x="191" y="370" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_0dw1gfd_di" bpmnElement="Flow_0dw1gfd">
        <di:waypoint x="241" y="410" />
        <di:waypoint x="345" y="410" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_0vtrbcl_di" bpmnElement="Flow_0vtrbcl">
        <di:waypoint x="395" y="410" />
        <di:waypoint x="540" y="410" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_0g0qdv8_di" bpmnElement="Flow_0g0qdv8">
        <di:waypoint x="370" y="435" />
        <di:waypoint x="370" y="490" />
        <di:waypoint x="191" y="490" />
        <di:waypoint x="191" y="530" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_1fpc6bn_di" bpmnElement="Flow_1fpc6bn">
        <di:waypoint x="241" y="570" />
        <di:waypoint x="320" y="570" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_16h9ynn_di" bpmnElement="Flow_16h9ynn">
        <di:waypoint x="370" y="610" />
        <di:waypoint x="370" y="640" />
        <di:waypoint x="191" y="640" />
        <di:waypoint x="191" y="670" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_1bwkxbo_di" bpmnElement="Flow_1bwkxbo">
        <di:waypoint x="241" y="710" />
        <di:waypoint x="320" y="710" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_1hfciq2_di" bpmnElement="Flow_1hfciq2">
        <di:waypoint x="420" y="710" />
        <di:waypoint x="530" y="710" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_0m5auoc_di" bpmnElement="Flow_0m5auoc">
        <di:waypoint x="630" y="710" />
        <di:waypoint x="750" y="710" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_1g97zx3_di" bpmnElement="Flow_1g97zx3">
        <di:waypoint x="800" y="750" />
        <di:waypoint x="800" y="800" />
        <di:waypoint x="191" y="800" />
        <di:waypoint x="191" y="850" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_18zcwiw_di" bpmnElement="Flow_18zcwiw">
        <di:waypoint x="241" y="890" />
        <di:waypoint x="320" y="890" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_1nq1i8n_di" bpmnElement="Flow_1nq1i8n">
        <di:waypoint x="370" y="930" />
        <di:waypoint x="370" y="992" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_181n368_di" bpmnElement="Flow_181n368">
        <di:waypoint x="241" y="250" />
        <di:waypoint x="345" y="250" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_1tg2ufs_di" bpmnElement="Flow_1tg2ufs">
        <di:waypoint x="395" y="250" />
        <di:waypoint x="530" y="250" />
      </bpmndi:BPMNEdge>
      <bpmndi:BPMNEdge id="Flow_178xmy2_di" bpmnElement="Flow_178xmy2">
        <di:waypoint x="530" y="120" />
        <di:waypoint x="191" y="120" />
        <di:waypoint x="191" y="210" />
      </bpmndi:BPMNEdge>
    </bpmndi:BPMNPlane>
  </bpmndi:BPMNDiagram>
</bpmn:definitions>

```


